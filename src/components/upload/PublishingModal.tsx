...
interface PublishingModalProps {
  ...
  onClose?: () => void; // Only available for error states
}
...
const PublishingModal: React.FC<PublishingModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col items-center justify-center">
      {/* Close button - only show for error states */}
      {isSuccess === false && onClose && (
        <button
          onClick={onClose}
      ...
      <div className="flex flex-col items-center justify-center text-center max-w-2xl mx-auto px-8">
        {/* Waawave Logo */}
        <div className="mb-16">
          <WaawaveLogo className="h-12 text-black" />
        </div>
        ...
        <div className="space-y-6">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">
            {isSuccess === null && "Hang tight! We're publishing your session."}
            {isSuccess === true && "Your session has been successfully published!"}
            {isSuccess === false && "Publication Failed"}
          </h1>
          
          <p className="text-lg md:text-xl text-gray-600 leading-relaxed max-w-lg mx-auto">
            {isSuccess === null && "Please don't close this tab or refresh the page while we work. This may take a few minutes."}
            {isSuccess === true && "Surfers can now discover and purchase your amazing photos and videos. Redirecting you to your session..."}
        ...
        {isSuccess === true && (
          <div className="mt-12 bg-green-50 border border-green-200 rounded-lg p-6 max-w-md">
            <div className="text-green-800 text-sm space-y-2">
              <p className="font-medium">What happens next:</p>
              <ul className="space-y-1 text-left">
                <li>• Your session is now live on Waawave</li>
                <li>• Surfers can browse and purchase your media</li>
                <li>• You'll earn 80% from each sale</li>
                <li>• Track your earnings in your photographer dashboard</li>
              </ul>
            </div>
          </div>
        )}
